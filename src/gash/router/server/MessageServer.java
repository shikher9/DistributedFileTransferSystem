...
public class MessageServer {
    ...
    private static ServerInfo si = null;
    protected gash.impl.raft.manager.HeartbeatManager heartbeatMgr;
	protected gash.impl.raft.manager.RaftElectionManager electionMgr;
	protected gash.impl.raft.manager.NodeDataManager nodeDataManager;
	// public static final String sPort = "port";
    ...
    public void startServer() {
    	...
    	//set secret
    	si.setSecret(123456789);
    	//set current node id
    	si.setNodeId(conf.getNodeId());
    		
        StartWorkCommunication comm = new StartWorkCommunication(conf,si);
        ...
        if (!conf.isInternalNode()) {
            StartCommandCommunication comm2 = new StartCommandCommunication(conf,si);
            logger.info("Command starting");

            if (background) {
                Thread cthread2 = new Thread(comm2);
                cthread2.start();
            } else //runs when first time server starts
            {
                comm2.run();
            }
        }
        
		RaftTimer raftTimer = RaftTimer.getInstance();
		raftTimer.SetServerInfo(si);
    ...
    private static class StartCommandCommunication implements Runnable {
        ...
        public StartCommandCommunication(RoutingConf conf, ServerInfo si) {
            this.conf = conf;
            this.serverInfo = si;
        }
        ...
        public void run() {
            ...
            try {
                ...
                b.childHandler(new CommandInit(conf, compressComm,serverInfo));
    ...
    private static class StartWorkCommunication implements Runnable {
        ...
        //queue for rerouting client messages meant for another server,
        LinkedBlockingDeque<QueueItem> pendingQueue;

        public StartWorkCommunication(RoutingConf conf, ServerInfo si) {
            ...
            pendingQueue = new LinkedBlockingDeque<QueueItem>();

            state = new ServerState();
            state.setConf(conf);

            state.setPendingQueue(pendingQueue);

            //TaskList tasks = new TaskList(new NoOpBalancer());
            //state.setTasks(tasks);
             si.setPendingQueue(pendingQueue);
            EdgeMonitor emon = new EdgeMonitor(state,si);
        ...
        public void run() {
            ...
            try {
                ...
                boolean compressComm = false;
                b.childHandler(new WorkInit(state, compressComm,si));
                ...
                //start comm worker
                CommWorker worker = new CommWorker(pendingQueue);
	...
	private void startManagers() {
		if (conf == null)
			return;
		nodeDataManager = gash.impl.raft.manager.NodeDataManager.initManager(conf);
		nodeDataManager.getNodeData().setNodeStatus(gash.impl.raft.manager.NodeData.RaftStatus.FOLLOWER);
		logger.info("After discrete queue");
    	electionMgr = gash.impl.raft.manager.RaftElectionManager.initManager(conf);
		heartbeatMgr = gash.impl.raft.manager.HeartbeatManager.initManager(conf);
		heartbeatMgr.start();
